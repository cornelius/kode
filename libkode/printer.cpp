...
QString Printer::classHeader( const Class &classObject, bool publicMembers )
{
  Code code;
  ...
  QString txt = "class " + d->mStyle.className( classObject.name() );
...
QString Printer::classImplementation( const Class &classObject )
{
  Code code;
  ...
  for ( it = functions.begin(); it != functions.end(); ++it ) {
    ...
    code += functionSignature( f, classObject.name(), true );
  ...
  if ( classObject.useDPointer() && !classObject.memberVariables().isEmpty() ) {
    ...
    code += functionSignature( cc, classObject.name(), true );
    ...
    code += functionSignature( op, classObject.name(), true );
    code += '{';
