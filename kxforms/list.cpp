    ...
    : GuiElement(parent, m, p)
{
    ...
    mFilterEdit->hide();
    connect(mFilterEdit, SIGNAL(textChanged(QString)), mProxyModel,
    ...
    mView->header()->hide();
    connect(mView, SIGNAL(doubleClicked(const QModelIndex &)), SLOT(editItem()));
...
void List::deleteItem()
{
    ...
    int result = KMessageBox::warningContinueCancel(
            mWidget, i18n("Delete item '%1'?", item->data(0).toString()));
...
{
    QModelIndex index = mProxyModel->mapToSource(selectedItem());
    if (!index.isValid() || index.row() == mModel->rowCount() - 1)
        return;
...
QStringList List::itemLabels(const ItemClass &itemClass, const QDomElement &itemElement)
{
    ...
    foreach (QDomNode labelDom, itemClass.labelDoms()) {
        QString label;
        for (n = labelDom.firstChild(); !n.isNull(); n = n.nextSibling()) {
            if (n.isText()) {
