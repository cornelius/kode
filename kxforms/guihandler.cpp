...
GuiHandler::GuiHandler(Manager *m) : mManager(m)
{
...
void GuiHandler::addElement(QLayout *l, QWidget *label, QWidget *widget, int x, int y, int width,
                            int height, GuiElement::Properties *prop, bool indented) const
{
    if (prop->layoutStyle == GuiElement::Horizontal) {
        ...
        if (label) {
            //       kDebug() <<"Adding label at: (" << y <<"," << 2*x <<")";
            //       kDebug() <<"Adding widget at: (" << y <<"," << (2*x)+1 <<") (" << height <<","
            //       << (2*width)-1  <<")";
            ...
            //       kDebug() <<"Adding widget at: (" << y <<"," << 2*x <<") (" << height <<"," <<
            //       2*width  <<")";
            if (indented) {
                QHBoxLayout *hbl = new QHBoxLayout();
                hbl->addSpacing(40);
        ...
        //     kDebug() <<"Adding block at: (" << y <<"," << (2*x) <<") (" << height <<"," <<
        //     2*width  <<")";
