...
Reference::Segment::Segment() : mCount(1), mIsAttribute(false) {}
...
void Reference::Segment::fromString(const QString &str)
{
    ...
    if (pos1 >= 0)
        mName = str.mid(startPos, pos1 - startPos);
    else
        mName = str.mid(startPos);
    ...
    if (pos1 >= 0 && pos2 > pos1) {
        ...
        if (!ok)
            kError() << "Illegal count in reference '" << str << "'";
    } else {
        mCount = 1;
    }
}
...
{
    return mName == s.mName && mCount == s.mCount && mIsAttribute == s.mIsAttribute;
}
...
Reference::Reference() : mValid(true)
{
    mAbsolute = false;
}
...
bool Reference::operator==(const Reference &ref) const
{
    ...
    if (it != mSegments.end() || it2 != ref.mSegments.end())
        return false;
...
bool Reference::matches(const Reference &ref, bool exact, bool pathOnly) const
{
    ...
    while (it != mSegments.constEnd() && it2 != list2.constEnd()) {
        //     kDebug() <<"Matching:" << (*it).name() <<":" << (*it).isAttribute() <<":" <<
        //     (*it).count(); kDebug() <<"With:" << (*it2).name() <<":" << (*it2).isAttribute()
        //     <<":" << (*it2).count();
        bool segmentsMatched = false;
        segmentsMatched =
                ((*it).name() == ((*it2).name()) && ((*it).isAttribute() == (*it2).isAttribute()));
    ...
    for (it = mSegments.constBegin(); it != mSegments.constEnd(); ++it) {
        ...
        if (result.isNull()) {
            ...
            if (doc.documentElement().tagName() == segment.name()) {
                ...
                kError() << "Document element '" << doc.documentElement().tagName() << "' isn't '"
                         << segment.name() << "'";
                return QDomElement();
            }
        }
        ...
        if (n.isNull()) {
            kError() << "Reference::apply(): Unable to find element '" << segment.toString() << "'";
            return QDomElement();
        }
    }
...
{
    kDebug() << "Reference::applyElement()" << toString() << "Context:" << context.tagName();

    QDomElement result = context;

    if (mSegments.count() == 1 && lastSegment().name() == ".")
        return result;
