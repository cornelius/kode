...
void EditorWidget::init()
{
    ...
    mShowHintsButton->setIcon(
            KIconLoader::global()->loadIcon("document-preview", KIconLoader::NoGroup, 32));
    ...
    mSaveHintsAsButton->setIcon(
            KIconLoader::global()->loadIcon("document-save-as", KIconLoader::NoGroup, 32));
    ...
    mEditDefaultsButton->setIcon(
            KIconLoader::global()->loadIcon("configure", KIconLoader::NoGroup, 32));
    ...
    mExitButton->setIcon(
            KIconLoader::global()->loadIcon("dialog-cancel", KIconLoader::NoGroup, 32));
    ...
    mEditButton->setIcon(
            KIconLoader::global()->loadIcon("document-properties", KIconLoader::NoGroup, 32));
    mEditButton->hide();
...
void EditorWidget::setGuiElements(const GuiElement::List &list)
{
    foreach (GuiElement *e, list) {
        ...
        QPoint labelWidgetPos = e->labelWidget()
                ? e->labelWidget()->mapToGlobal(QPoint(0, 0)) - mapToGlobal(QPoint(0, 0))
                : widgetPos;
...
void EditorWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (mInDrag && !mDraggedElement
        && (mDragPoint - pos).manhattanLength() > QApplication::startDragDistance()) {
        foreach (const QRect &dragR, mElementMap) {
            if (dragR.contains(mDragPoint)) {
                if (!mDraggedElement || mElementMap[mDraggedElement].width() > dragR.width()
                    || mElementMap[mDraggedElement].height() > dragR.height()) {
                    mDraggedElement = mElementMap.key(dragR);
                }
            }
        }
        QPixmap cursorPixmap = QPixmap::grabWidget(mGui, mElementMap[mDraggedElement])
    ...
    foreach (const QRect &r, mElementMap) {
        if (r.contains(pos)) {
            if (!newElement || mElementMap[newElement].width() > r.width()
...
void EditorWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (mInDrag) {
        ...
        if (mDraggedElement && mHoveredElement && mDraggedElement != mHoveredElement
...
void EditorWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (mInSelection) {
        if (mHoveredElement != mActiveElement) {
            if (mHoveredElement
                && mActiveElement->properties()->group != mHoveredElement->properties()->group)
                printMessage(&p, mElementMap[mHoveredElement],
                             i18n("Groups do not match: %1 != %2",
...
void EditorWidget::drawGlobalInterface(QPainter *p)
{
    ...
    QRect r(width() - mInterfaceWidget->width() - 15, 5, mInterfaceWidget->width() + 10,
            mInterfaceWidget->height() + 10);
