...
void Editor::setupActions()
{
    ...
    connect(a, SIGNAL(hintGenerated(const Hint &)), SLOT(applyHint(const Hint &)));
...
void Editor::saveHints()
{
    if (mHintsUrl.isEmpty() || !mHintsUrl.isValid()) {
        KMessageBox::sorry(mEditorWidget, i18n("Invalid Url '%1'.", mHintsUrl.prettyUrl()));
        return;
    } else {
        QFile file(mHintsUrl.path());
        if (!file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
            KMessageBox::sorry(mEditorWidget, i18n("Can not open '%1'.", mHintsUrl.prettyUrl()));
            return;
        }
...
{
    KUrl url = KFileDialog::getSaveUrl(KUrl(), QString(), mEditorWidget, i18n("Select Hints File"));
    if (!url.isEmpty() && url.isValid()) {
        mHintsUrl = url;
        saveHints();
    } else {
        KMessageBox::sorry(mEditorWidget, i18n("Invalid Url '%1'.", url.prettyUrl()));
    }
}
