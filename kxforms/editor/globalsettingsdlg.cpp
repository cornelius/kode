...
GroupDialog::GroupDialog(const QString &caption, const QString &id, const QString &title,
                         QWidget *parent)
    : KDialog(parent)
{
    setCaption(caption);
    ...
    : KDialog(parent), mManager(manager)
{
    ...
    tabWidget->addTab(formPage,
                      i18n("%1 options", mManager->currentGui()->ref().lastSegment().toString()));
...
void GlobalSettingsDialog::accept()
{
    ...
    globalHint.setValue(Hint::ReadOnly,
                        mReadOnlyCheckBox->checkState() == Qt::Checked ? "true" : "false");
...
void GlobalSettingsDialog::slotAddGroup()
{
    ...
    if (dlg->exec() == QDialog::Accepted) {
        ...
        mGroupWidget->insertTopLevelItem(mGroupWidget->topLevelItemCount(),
                                         new QTreeWidgetItem(data));
    }
    dlg->deleteLater();
}
...
void GlobalSettingsDialog::slotEditGroup()
{
    ...
    if (dlg->exec() == QDialog::Accepted) {
        QStringList data;
        data << dlg->id() << dlg->title();
        mGroupWidget->insertTopLevelItem(mGroupWidget->indexOfTopLevelItem(item),
                                         new QTreeWidgetItem(data));
...
void GlobalSettingsDialog::slotDeleteGroup()
{
    if (mGroupWidget->currentItem())
        mGroupWidget->takeTopLevelItem(
                mGroupWidget->indexOfTopLevelItem(mGroupWidget->currentItem()));
}
