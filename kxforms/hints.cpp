...
Hint::Hint(const Reference &ref) : mRef(ref)
{
...
void Hints::extractHints(const QList<QDomElement> &annotations, const Reference &refPrefix)
{
    ...
    foreach (Hint h, mHints) {
        ...
        foreach (Hint::Type t, elementValues.keys()) {
            ...
            foreach (QDomElement e, list) {
                ...
                e.save(stream, 0);
                s += QString("&nbsp;&nbsp;&nbsp;%1: \"%2\"<br>")
                             .arg(Hint::nameForType(t), Qt::escape(tmp.simplified()));
            }
        }
    }
