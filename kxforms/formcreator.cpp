...
{
    if (mCollapsedForms.contains(element.name()))
        return;
    ...
    foreach (Schema::Relation r, element.attributeRelations()) {
        ...
        if (a.type() == Schema::Attribute::String) {
            ...
            foreach (QString value, a.enumerationValues()) {
                ...
                if (hint.isValid())
                    itemLabel = hint.enumValue(value);
                if (itemLabel.isEmpty())
                    itemLabel = humanizeString(value);
                                   ...
                                   Reference path)
{
    ...
    kDebug() << element.name();
    if (!topLevel && !element.mixed() && !choiceOnly(element)) {
    ...
    Reference fullRef = path.matches(Reference(element.name()), false)
            ? path
            : path + Reference(element.name());
    ...
    if (element.text()) {
        ...
        for (int i = 0; i < relations.size(); ++i) {
            ...
            if (r.isList()) {
                ...
                if (hint.isValid() && hint.elements(Hint::ListItemLabel).size() > 0) {
                    ...
                    if (listElement.type() == Schema::Node::NormalizedString
                        ...
                        || (listElement.type() == Schema::Node::ComplexType
                            && listElement.mixed())) {
                        item->tag("itemlabel",
                                  createListItemLabel(Reference("."), path, listItemType));
                        listItemType.clear();
                        ...
                        if (subElements.size() == 0) {
                            ...
                            if (subElements.size() == 0 || !nameAttribute) {
                                while (subElementCnt < 3) {
                                    subElements =
                                            collectSubElements(listElement, path, depth++, true);
                ...
                if (list
                    && ((i == relations.size() - 1)
                        || listItemClassCount == r.choice().count('+') + 1
                        || (i < relations.size() - 1
                            && (relations[i + 1].choice() != r.choice()
                                || !relations[i + 1].isList())))) {
...
QString FormCreator::createListItemLabel(const Reference &r, const Reference &path,
                                         const QString &itemType)
{
    ...
    if (itemLabel.isEmpty()) {
        if (itemElement.type() == Schema::Node::String
            || (itemElement.type() == Schema::Node::ComplexType && itemElement.mixed())) {
            itemLabel += typeString;
            itemLabel += QString("<itemLabelArg ref=\"%1\" truncate=\"40\"/>")
                                 .arg((path + r).toString());
        } else if (itemElement.type() == Schema::Node::NormalizedString
                   || itemElement.type() == Schema::Node::Token) {
            itemLabel += typeString;
...
QList<Reference> FormCreator::collectSubElements(const Schema::Element &element,
                                                 const Reference &path, int depth,
                                                 bool includeAttributes)
{
    ...
    foreach (Schema::Relation r, element.elementRelations()) {
        ...
        foreach (Reference r,
                 collectSubElements(subElement, path + Reference(subElement.name()), depth,
                                    includeAttributes)) {
            if (!list.contains(r))
                list.append(r);
        }
...
QString FormCreator::getLabel(const Reference &ref, const QString &fallback, bool pluralize)
{
    //   kDebug() << ref.toString();
    ...
    if (hint.isValid())
        label = hint.value(Hint::Label);

    if (label.isEmpty())
        label = humanizeString(fallback, pluralize);
...
void FormCreator::applyCommonHints(XmlBuilder *xml, const Reference &ref)
{
    ...
    if (hint.hasValue(Hint::GroupReference)) {
        if (!properties)
            properties = xml->tag("properties");
        if (!layout)
            layout = properties->tag("layout");
