...
Pattern::Pattern() : optional(false), zeroOrMore(false), oneOrMore(false), choice(false) {}
...
Element::Element() : hasText(false), isEmpty(false) {}
...
bool ParserRelaxng::parseAttribute(const QDomElement &attributeElement, Attribute *a)
{
...
bool ParserRelaxng::parseElement(const QDomElement &elementElement, Element *e, Pattern pattern)
{
    if (mVerbose) {
    ...
    for (e1 = elementElement.firstChildElement(); !e1.isNull(); e1 = e1.nextSiblingElement()) {
        if (e1.tagName() == "element") {
            ...
            Pattern p = pattern;
            if (e1.tagName() == "optional")
                p.optional = true;
            else if (e1.tagName() == "zeroOrMore")
                p.zeroOrMore = true;
            else if (e1.tagName() == "oneOrMore")
                p.oneOrMore = true;
            else if (e1.tagName() == "choice")
                p.choice = true;
            else {
    ...
    while (it != s->references.end()) {
        ...
        if (it1 != mDefinitionMap.constEnd()) {
            ...
            for (it4 = elements.constBegin(); it4 != elements.constEnd(); ++it4) {
                ...
                for (it3 = d->attributes.constBegin(); it3 != d->attributes.constEnd(); ++it3) {
                    Attribute *a = *it3;
    ...
    for (Attribute *attribute : qAsConst(e->attributes)) {
        ...
        if (!mDocument.attribute(id).isValid()) {
            Schema::Attribute relatedAttribute =
                    convertToSchemaAttribute(schemaElement.identifier(), attribute);
        }
...
Schema::Relation ParserRelaxng::convertToRelation(const Pattern &pattern, const QString &id)
{
...
Schema::Attribute ParserRelaxng::convertToSchemaAttribute(const QString &path, Attribute *a)
{
