    ...
    : ComplexInputField(name, type)
{
    ...
    for (elemIt = elements.constBegin(); elemIt != elements.constEnd(); ++elemIt) {
        ...
        InputField *field = InputFieldFactory::self()->createField(
                (*elemIt).name(), (*elemIt).type().qname(), isList);
        if (!field) {
            qDebug("ComplexBaseInputField: Unable to create field of type %s",
    ...
    for (attrIt = attributes.constBegin(); attrIt != attributes.constEnd(); ++attrIt) {
        InputField *field =
                InputFieldFactory::self()->createField((*attrIt).name(), (*attrIt).type().qname());
        if (!field) {
            qDebug("ComplexBaseInputField: Unable to create field of type %s",
                   qPrintable(type->baseTypeName().qname()));
        } else {
...
void ComplexBaseInputField::setXMLData(const QDomElement &element)
{
    if (mName != element.tagName()) {
        qDebug("ComplexBaseInputField: Wrong dom element passed: expected %s, got %s",
    ...
    if (mType->isArray()) {
        ...
        while (!node.isNull()) {
            ...
            if (!child.isNull()) {
                ...
                if (!field) {
                    qDebug("ComplexBaseInputField: Child field %s does not exists",
