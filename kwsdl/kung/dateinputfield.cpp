...
DateInputField::DateInputField(const QString &name, const XSD::SimpleType *type)
    : SimpleInputField(name, type), mValue(QDate::currentDate())
{
}
...
void DateInputField::setXMLData(const QDomElement &element)
{
    if (mName != element.tagName()) {
        qDebug("DateInputField: Wrong dom element passed: expected %s, got %s", qPrintable(mName),
...
QWidget *DateInputField::createWidget(QWidget *parent)
{
    ...
    connect(mInputWidget, SIGNAL(dateChanged(QDate)), this, SLOT(inputChanged(QDate)));
