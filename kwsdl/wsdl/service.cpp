...
Service::Service(const QString &nameSpace) : Element(nameSpace) {}
    ...
    while (!child.isNull()) {
        ...
        if (tagName.localName() == "port") {
            ...
            context->messageHandler()->warning(
                    QString("Service: unknown tag %1").arg(child.tagName()));
        }
...
void Service::saveXML(ParserContext *context, const Binding::List *bindings, QDomDocument &document,
