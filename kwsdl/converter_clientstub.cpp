                                          ...
                                          const Binding &binding, KODE::Class &newClass)
{
            ...
            for (int i = 0; i < parts.count(); ++i) {
                if (parts[i].name() == header.part()) {
                    ...
                    if (!type.isEmpty()) {
                        ...
                        soapHeaderName =
                                mNSManager.fullName(element.nameSpace(), element.localName());
                    }
    ...
    for (it = parts.begin(); it != parts.end(); ++it) {
        ...
        if (!type.isEmpty()) {
            ...
            callFunc.addArgument(mTypeMap.localTypeForElement((*it).element()) + " *"
                                 + mNameMapper.escape(lowerName));
        }
    }

    KODE::Code code;
    code += "QDomDocument doc;";
    code += "doc.appendChild( doc.createProcessingInstruction( \"xml\", \"version=\\\"1.0\\\" "
            "encoding=\\\"UTF-8\\\"\" ) );";
    ...
    if (soapStyle == SoapBinding::RPCStyle) {
        code += "QDomElement wrapper = doc.createElement( \""
                + mNSManager.fullName(soapBody.nameSpace(), operation.name()) + "\" );";
        // encodingStyle can be empty or not existent, we have to differ here
        if (!soapBody.encodingStyle().isNull())
            code += "wrapper.setAttribute( \"SOAP-ENV:encodingStyle\", \""
    ...
    for (it = parts.begin(); it != parts.end(); ++it) {
        ...
        if (soapStyle == SoapBinding::RPCStyle) {
            ...
            if (!type.isEmpty()) {
                ...
                name = mNSManager.fullName((*it).element().nameSpace(),
                                           (*it).element().localName());
            }
        }

        code += "Serializer::marshal( doc, " + parentNode + ", \"" + name + "\", "
                + mNameMapper.escape(lowerName) + ", " + noNamespace + " );";
                                           ...
                                           const Binding &binding, KODE::Class &newClass)
{
    ...
    respSignal.setDocs("This signal is emitted whenever the call to " + operationName
                       + "() succeeded.");
    ...
    errorSignal.setDocs("This signal is emitted whenever the call to " + operationName
                        + "() failed.");

    newClass.addFunction(errorSignal);

    // result slot
    KODE::Function respSlot(operationName + "Slot", "void",
                            KODE::Function::Slot | KODE::Function::Private);
    ...
    // error slot
    KODE::Function errorSlot(operationName + "ErrorSlot", "void",
                             KODE::Function::Slot | KODE::Function::Private);
