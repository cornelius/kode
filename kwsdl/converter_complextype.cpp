...
void Converter::createComplexTypeSerializer(const XSD::ComplexType *type)
{
    ...
    marshalCode += "QDomElement root = doc.createElement( noNamespace ? name : \"" + typePrefix
            + ":\" + name );";
    ...
    if (type->baseTypeName() != XmlAnyType && !type->baseTypeName().isEmpty() && !type->isArray()) {
        ...
        marshalCode += "Serializer::marshal( doc, parentElement, QString(), (" + baseType
                + "*)value, noNamespace );";
    }
    ...
    for (elemIt = elements.constBegin(); elemIt != elements.constEnd(); ++elemIt) {
        ...
        if ((*elemIt).maxOccurs() > 1) {
            ...
            marshalCode += '{';
            marshalCode.indent();
            marshalCode += "const QList<" + typeName + "*> *list = value->"
                    + mNameMapper.escape(lowerName) + "();";
            marshalCode.newLine();
            marshalCode += "QList<" + typeName + "*>::ConstIterator it;";
            marshalCode += "for ( it = list->begin(); it != list->end(); ++it ) {";
            marshalCode.indent();
            marshalCode += "Serializer::marshal( doc, parentElement, \"" + typePrefix + ":"
                    + (*elemIt).name() + "\", *it, false );";
            marshalCode.unindent();
            marshalCode += '}';
            marshalCode.unindent();
            marshalCode += '}';
            ...
            demarshalStartCode +=
                    "QList<" + typeName + "*> *" + listName + " = new QList<" + typeName + "*>();";
            ...
            marshalCode += "Serializer::marshal( doc, parentElement, \"" + (*elemIt).name()
                    + "\", value->" + getter.name() + "(), false );";
    ...
    for (attrIt = attributes.constBegin(); attrIt != attributes.constEnd(); ++attrIt) {
        ...
        if (isArray) {
            ...
            marshalCode += "QDomElement element = doc.createElement( noNamespace ? name : \""
                    + typePrefix + ":\" + name );";
            marshalCode += "element.setAttribute( \"" + mNSManager.schemaInstancePrefix()
                    + ":type\", \"" + mNSManager.soapEncPrefix() + ":Array\" );";
            marshalCode += "element.setAttribute( \"" + mNSManager.soapEncPrefix()
                    + ":arrayType\", \"" + typePrefix + ":" + typeName
            ...
            demarshalCode.indent();
            demarshalCode += typeName + " *item = new " + typeName + "();";
            demarshalCode += "Serializer::demarshalValue( element.attribute( \"" + (*attrIt).name()
                    + "\" ), item );";
            demarshalCode += "value->" + setter.name() + "( item );";
            demarshalCode.unindent();
            demarshalCode += '}';
