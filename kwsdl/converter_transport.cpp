...
void Converter::createKDETransport()
{
    ...
    queryCode.newLine();
    queryCode += "KIO::TransferJob* job = KIO::http_post( KUrl( " + url.name()
    ...
    queryCode += "job->addMetaData( \"content-type\", \"Content-Type: application/xml; "
                 "charset=utf-8\" );";
    ...
    queryCode += '}';
    queryCode.newLine();
    queryCode += "connect( job, SIGNAL( data( KIO::Job*, const QByteArray& ) ), this, SLOT( "
    ...
    slotDataCode +=
            "memcpy( " + slotDataVar.name() + ".data() + oldSize, data.data(), data.size() );";
    ...
    slotResultCode += "emit result( QString::fromUtf8( " + slotDataVar.name() + ".data(), "
            + slotDataVar.name() + ".size() ) );";
...
void Converter::createQtTransport()
{
    ...
    code.newLine();
    code += "connect( " + httpVar.name() + ", SIGNAL( requestFinished( int, bool ) ), this, SLOT( "
            + finishedSlot.name() + "( int, bool ) ) );";
    ctor.setBody(code);
    ...
    code.unindent();
    code.newLine();
    code += idVar.name() + " = " + httpVar.name() + "->request( header, message.toUtf8(), &"
            + bufferVar.name() + " );";
    query.setBody(code);
