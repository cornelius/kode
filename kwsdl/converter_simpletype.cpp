    ...
    if (type->subType() == XSD::SimpleType::TypeRestriction) {
        ...
        if (type->facetType() & XSD::SimpleType::ENUM) {
            ...
            classDocumentation += "Whenever you have to pass an object of type " + newClass.name()
                    + " you can also pass the enum directly (e.g. someMethod( " + newClass.name()
                    + "::" + enums.first() + "  )).";
         ...
         */
        if (type->baseTypeName() != XmlAnyType && !type->baseTypeName().isEmpty()
            ...
            classDocumentation += "Whenever you have to pass an object of type " + newClass.name()
                    + " you can also pass the value directly (e.g. someMethod( (" + typeName
                    + "*)value  )).";
            // include header
...
void Converter::createSimpleTypeSerializer(const XSD::SimpleType *type)
{
    ...
    if (type->subType() == XSD::SimpleType::TypeRestriction) {
        ...
        if (type->facetType() & XSD::SimpleType::ENUM) {
            ...
            marshalCode += "QDomElement root = doc.createElement( noNamespace ? name : \""
                    + typePrefix + ":\" + name );";
            marshalCode += "root.setAttribute( \"" + mNSManager.schemaInstancePrefix()
                    + ":type\", \"" + typePrefix + ':' + type->name() + "\" );";
            ...
            marshalCode += "parentElement.appendChild( doc.createTextNode( "
                           "Serializer::marshalValue( value ) ) );";
        ...
        marshalCode += "if ( !name.isEmpty() ) {";
        marshalCode.indent();
        marshalCode += "QDomElement element = doc.createElement( noNamespace ? name : \""
                + mNSManager.prefix(type->listTypeName().nameSpace()) + ":\" + name );";
        ...
        demarshalCode +=
                "const QStringList list = parent.text().split( \" \", QString::SkipEmptyParts );";
...
static KODE::Code createRangeCheckCode(const XSD::SimpleType *type, const QString &variableName,
                                       KODE::Class &parentClass)
{
    KODE::Code code;
    ...
    */
    ...
    if (type->facetType() & XSD::SimpleType::MININC)
        code += "rangeOk = rangeOk && (" + variableName
                + " >= " + QString::number(type->facetMinimumInclusive()) + ");";
    if (type->facetType() & XSD::SimpleType::MINEX)
        code += "rangeOk = rangeOk && (" + variableName + " > "
                + QString::number(type->facetMinimumExclusive()) + ");";
    if (type->facetType() & XSD::SimpleType::MAXINC)
        code += "rangeOk = rangeOk && (" + variableName
                + " <= " + QString::number(type->facetMaximumInclusive()) + ");";
    if (type->facetType() & XSD::SimpleType::MINEX)
        code += "rangeOk = rangeOk && (" + variableName + " < "
                + QString::number(type->facetMaximumExclusive()) + ");";

    if (type->facetType() & XSD::SimpleType::LENGTH)
        code += "rangeOk = rangeOk && (" + variableName
                + ".length() == " + QString::number(type->facetLength()) + ");";
    if (type->facetType() & XSD::SimpleType::MINLEN)
        code += "rangeOk = rangeOk && (" + variableName
                + ".length() >= " + QString::number(type->facetMinimumLength()) + ");";
    if (type->facetType() & XSD::SimpleType::MAXLEN)
        code += "rangeOk = rangeOk && (" + variableName
                + ".length() <= " + QString::number(type->facetMaximumLength()) + ");";
