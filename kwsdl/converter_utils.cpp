...
void Converter::createUtils()
{
    ...
    TypeEntry types[] = {
        ...
        { "QByteArray", "base64Binary", "QString::fromUtf8( value->toBase64() )",
          "QByteArray::fromBase64( str.toUtf8() )" },
        { "QDateTime", "dateTime", "value->toString( Qt::ISODate )",
          "QDateTime::fromString( str, Qt::ISODate )" },
        { "QDate", "date", "value->toString( Qt::ISODate )",
          "QDate::fromString( str, Qt::ISODate )" },
        { "QTime", "time", "value->toString( Qt::ISODate )",
          "QTime::fromString( str, Qt::ISODate )" }
    };
    ...
    for (uint i = 0; i < sizeof(types) / sizeof(TypeEntry); ++i) {
        ...
        code.indent();
        code += "QDomElement element = doc.createElement( noNamespace ? name : \""
                + mNSManager.schemaPrefix() + ":\" + name );";
        code += "element.setAttribute( \"" + mNSManager.schemaInstancePrefix() + ":type\", \""
                + mNSManager.schemaPrefix() + ":" + entry.xsdType + "\" );";
...
void Converter::createSoapUtils()
{
    ...
    soapFault.setDocs("This class encapsulates error information.\n"
