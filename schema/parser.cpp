...
Annotation::List Parser::parseAnnotation(ParserContext *, const QDomElement &element)
{
    ...
    QDomElement e;
    for (e = element.firstChildElement(); !e.isNull(); e = e.nextSiblingElement()) {
...
void Parser::parseCompositor(ParserContext *context, const QDomElement &element, ComplexType &ct)
{
    QName name = element.tagName();
                ...
                if (isChoice) {
                    ...
                    if (isSequence) {
                        ...
                        if (childElement.hasAttribute("minOccurs")
                            ...
                            newElement = parseElement(context, childElement, ct.nameSpace(),
                                                      childElement);
                        } else {
                            newElement =
...
{
    for (int i = 0; i < d->mElements.count(); ++i) {
        if (d->mElements[i].nameSpace() == name.nameSpace()
            && d->mElements[i].name() == name.localName())
...
{
    for (int i = 0; i < d->mAttributes.count(); ++i) {
        if (d->mAttributes[i].nameSpace() == name.nameSpace()
            && d->mAttributes[i].name() == name.localName())
